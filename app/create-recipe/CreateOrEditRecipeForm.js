'use client'; // هذا السطر يجب أن يبقى هنا وهو أول سطر
...
export default function CreateOrEditRecipeForm() { // <<<<< تم تغيير اسم الدالة هنا
  ...
  useEffect(() => {
    setMounted(true);
    // تأكد من أننا نصل إلى searchParams.get('id') فقط بعد التحميل في المتصفح
    const idFromParams = rawSearchParams.get('id');
    setRecipeId(idFromParams);
  }, [rawSearchParams]);
  ...
  useEffect(() => {
    // هذا useEffect سيعمل فقط عندما يكون المكون قد تم تحميله في المتصفح و recipeId قد تم تحديده
    if (!mounted || recipeId === null) {
      // إذا كان recipeId null و mounted صحيحاً، فهذا يعني أنها صفحة وصفة جديدة
      if (mounted && recipeId === null) {
        setLoading(false);
      }
      return;
  ...
  }, [fetchRecipeData, recipeId, isAuthenticated, router, mounted]);
